...
class DatabaseService {
  ...
  constructor() {
    ...
    try {
      ...
      console.warn('Banco de dados: Supabase não configurado, usando localStorage como alternativa');
  ...
  private getLocalStorageData(): Transaction[] {
    try {
      const data = localStorage.getItem('transactions_fallback');
  ...
  private setLocalStorageData(transactions: Transaction[]): void {
    try {
      localStorage.setItem('transactions_fallback', JSON.stringify(transactions));
    } catch {
  ...
  async getAll(): Promise<ApiResponse<Transaction[]>> {
    try {
      ...
      if (this.useSupabase) {
        ...
        if (result.error) {
          console.warn('Banco de dados: Supabase falhou, usando localStorage como alternativa');
          this.useSupabase = false;
          transactions = this.getLocalStorageData();
        } else {
        ...
        if (error) {
          if (error.code !== 'PGRST116') {
            ...
            if (all.success && all.data) {
              ...
              this.setLocalStorageData(updated);
            }
      ...
      if (this.useSupabase) {
        ...
        if (error) {
          ...
          if (all.success && all.data) {
            ...
            this.setLocalStorageData(filtered);
